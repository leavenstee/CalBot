...
class HealthStore {
    ...
    fileprivate var requestAuthorizationTask: HealthStoreTask {
        get {
    ...
    func findStepCountHistory(_ dates: Int = 14) -> HealthStoreStepCountHistoryListTask {
    ...
    fileprivate var stepCountBackgroundUpdateQuery: HKObserverQuery?
    
    func setStepCountBackgroundUpdateHandler(_ updateHandler: (((Void)->Void, NSError?) -> Void)?) {
        if let handler = updateHandler {
            ...
            self.store.stop(self.stepCountBackgroundUpdateQuery!)!!
